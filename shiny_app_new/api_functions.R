...
# اصلاح‌شده: افزودن تبریز + اصلاح مسیر ذخیره خودکار
# ==============================
...
find_data_dir <- function() {
  ...
  #    چند کاندید رایج را امتحان می‌کنیم
...
download_historical_data <- function(station_id, start_date, end_date) {
  ...
  # ── مختصات ایستگاه‌ها ─────────────────────────────────────────────────────
  # نکته: این مختصات باید دقیقاً با STATIONS در global.R یکسان باشد
  # تا داشبورد بتواند داده‌های دانلودشده را به ایستگاه‌های صحیح وصل کند.
  coords <- list(
    ...
    tabriz  = c(lat = 38.0962, lon = 46.2738)   # ── افزوده‌شده ──
  )
...
# --------------------------------------------------
# Download and save all 5 cities
# --------------------------------------------------
download_all_cities <- function(start_date, end_date, out_dir = NULL) {

  # ── اگر out_dir مشخص نشد، خودکار پیدا کن ──────────────────────────────────
  if (is.null(out_dir)) {
    out_dir <- find_data_dir()
  }

  message("[download_all_cities] پوشه خروجی: ", out_dir)
  ...
  for (city in cities) {
    cat("\n==============================\n")
    cat("Downloading:", city, "\n")
    cat("==============================\n")

    df_city <- tryCatch(
      download_historical_data(city, start_date, end_date),
      error = function(e) {
    ...
    if (!is.null(df_city) && nrow(df_city) > 0) {
      ...
      write.csv(df_city, file_name, row.names = FALSE)
      message("Saved: ", file_name)

      all_data[[city]] <- df_city
    }
  }

  if (length(all_data) == 0) {
    stop("No city data was downloaded.")
  }
  ...
  message("Saved combined file: ", combined_file)
...
# RUN
# --------------------------------------------------
# نکته: این خط فقط وقتی اجرا میشه که فایل را source یا Run All کنید.
# در حالت shiny اپ، global.R این فایل را source نمی‌کند و فقط download_all_cities
# در دسترس است.
if (interactive()) {
  result <- download_all_cities(
